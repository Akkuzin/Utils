...
public class LevinshteinTransformation {
  ...
  @AllArgsConstructor
  public abstract static class BaseOperation<T> implements IOperation<T> {
  ...
  public static class ReplaceOperation<T> extends BaseOperation<T> {
    int place;
    T newValue;
    @Getter Function<List<T>, List<T>> action;
  ...
  @FieldDefaults(level = AccessLevel.PRIVATE, makeFinal = true)
  @ToString
  public static class InsertOperation<T> extends LevinshteinTransformation.BaseOperation<T> {
    int place;
    T newValue;
    @Getter Function<List<T>, List<T>> action;
  ...
  public static final LevenshteinParameters DEFAULT_PARAMETERS =
      LevenshteinParameters.builder()
          .insertWeight(10)
          .deleteWeight(10)
          .replaceWeight(19)
          .swapWeight(19)
  ...
  public static <T> Collection<IOperation<T>> levenshteinDistance(
      List<T> source, List<T> target, LevenshteinParameters params) {
    ...
    for (int j = 1; j <= targetLength; ++j) {
      for (int i = 1; i <= sourceLength; ++i) {
        ...
        int swapCost =
            (i > 1
                    && j > 1
                    && source.get(i - 1) == target.get(j - 2) //
                    && source.get(i - 2) == target.get(j - 1)) //
                ? distance[i - 2][j - 2] + params.swapWeight
                : Integer.MAX_VALUE;

        distance[i][j] = Collections.min(asList(deleteCost, insertCost, replaceCost, swapCost));
