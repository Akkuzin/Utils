...
public class GetterPropertyResolver<T> {
  ...
  public static <T> T getPropertyNameCapturer(Class<T> type) {
    ...
    Class<?> proxyType =
        builder
            .implement(aaa.lang.reflection.getter.PropertyNameCapturer.class)
            .defineField("propertyName", String.class, Visibility.PRIVATE)
            .defineField("propertyType", Class.class, Visibility.PRIVATE)
            .method(ElementMatchers.any())
            .intercept(
                MethodDelegation.to(
                    aaa.lang.reflection.getter.PropertyNameCapturingInterceptor.class))
            ...
            .make()
            .load(
                aaa.lang.reflection.getter.PropertyNameCapturer.class.getClassLoader(),
                ClassLoadingStrategy.Default.WRAPPER)
            .getLoaded();
    try {
      return ((Class<T>) proxyType).getDeclaredConstructor().newInstance();
    } catch (InstantiationException
        | IllegalAccessException
        | NoSuchMethodException
        | InvocationTargetException e) {
      throw new IllegalArgumentException(
          "Couldn't instantiate proxy for retrieval of method name ", e);
    }
  }
