...
public class ReflectionUtils {
  ...
  /**
   * Для некоторого класса (или интерфейса) определяет, каким классом был параметризован один из его
   * предков (реализующих классов) с generic-параметрами.
   *
  ...
  public static Class<?> getGenericParameterClass(
      final Class<?> actualClass, final Class<?> genericClass, final int parameterIndex) {
    ...
    // Будем подниматься по иерархии, пока не найдем интересующий нас класс.
    // В процессе поднятия мы будем сохранять в genericClasses все классы - они нам понадобятся при
    // спуске вниз.
    ...
    while (true) {
      ...
      if (isParameterizedType) {
        // Если предок - параметризованный класс, то запоминаем его - возможно он пригодится при
        // спуске вниз.
        genericClasses.push((ParameterizedType) currentType);
      } else {
        // В иерархии встретился непараметризованный класс.
        // Все ранее сохраненные параметризованные классы будут бесполезны.
        genericClasses.clear();
      }
      // Проверяем, дошли мы до нужного предка или нет.
      Type rawType =
          isParameterizedType ? ((ParameterizedType) currentType).getRawType() : currentType;
      if (!rawType.equals(genericClass)) {
        // genericClass не является непосредственным родителем для текущего класса. Поднимаемся по
        // иерархии дальше.
        clazz = (Class<?>) rawType;
      } else {
    ...
    while (result instanceof TypeVariable<?> && !genericClasses.empty()) {
      // Похоже, что параметр задан где-то ниже по иерархии, спускаемся ниже.
    ...
    if (result instanceof ParameterizedType) {
      // Сам параметр оказался параметризованным. Отбросим информацию о его параметрах, она нам не
      // нужна.
    ...
    if (!(result instanceof Class<?>)) {
      // Похоже, что параметр - массив, примитив, интерфейс или еще что-то, что не классом.
      throw new IllegalStateException(
  ...
  public static int getParameterTypeDeclarationIndex(final TypeVariable<?> typeVariable) {
    ...
    throw new IllegalStateException(
        "Argument " + typeVariable + " is not found in " + genericDeclaration + ".");
  }

  public static Type getGenericInterface(
      final Class<?> sourceClass, final Class<?> genericInterface) {
  ...
  private static Function createGetter(MethodHandles.Lookup lookup, MethodHandle getter) {
    return (Function)
        LambdaMetafactory.metafactory(
                ...
                MethodType.methodType(Object.class, Object.class), // Function.apply with erasure
                getter,
                getter.type())
            .getTarget()
            .invokeExact();
  ...
  @SneakyThrows
  public static BiConsumer createSetter(MethodHandles.Lookup lookup, MethodHandle setter) {
    Class<?> valueType = setter.type().parameterType(1);
    if (valueType.isPrimitive()) {
      if (valueType == boolean.class) {
        ObjBooleanConsumer consumer =
            (ObjBooleanConsumer)
                createSetterCallSite(lookup, setter, ObjBooleanConsumer.class, boolean.class)
                    .getTarget()
                    .invokeExact();
        ...
        ObjLongConsumer consumer =
            (ObjLongConsumer)
                createSetterCallSite(lookup, setter, ObjLongConsumer.class, long.class)
                    .getTarget()
                    .invokeExact();
        ...
        ObjDoubleConsumer consumer =
            (ObjDoubleConsumer)
                createSetterCallSite(lookup, setter, ObjDoubleConsumer.class, double.class)
                    .getTarget()
                    .invokeExact();
        ...
        ObjIntConsumer consumer =
            (ObjIntConsumer)
                createSetterCallSite(lookup, setter, ObjIntConsumer.class, int.class)
                    .getTarget()
                    .invokeExact();
      ...
      return (BiConsumer)
          createSetterCallSite(lookup, setter, BiConsumer.class, Object.class)
              .getTarget()
              .invokeExact();
    }
  }
  ...
  @SneakyThrows
  private static CallSite createSetterCallSite(
  ...
  public static <T, V> Function<T, V> asGetter(Member member) {
    if (member instanceof Method || member instanceof Field) {
      ...
      if (member instanceof Method method) {
        return (Function<T, V>) reflectGetter(lookup, method);
      }
      String name = capitalize(member.getName());
      Field field = (Field) member;
      try {
        return (Function<T, V>)
            reflectGetter(lookup, field.getDeclaringClass().getMethod("get" + name));
      } catch (NoSuchMethodException e) {
        return (Function<T, V>)
            reflectGetter(lookup, field.getDeclaringClass().getMethod("is" + name));
      }
    } else {
